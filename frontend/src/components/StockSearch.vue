...
const debouncedSearch = debounce(async (keyword: string) => {
  ...
  try {
    if (props.marketType === 'A') {
      ...
      // 限制只显示前10个结果
      results.value = searchResults.slice(0, 10);
    } else if (props.marketType === 'ETF') {
      // ETF基金搜索
      const searchResults = await apiService.searchFundsETF(keyword);
      // 限制只显示前10个结果
      results.value = searchResults.slice(0, 20);
    } else if (props.marketType === 'LOF') {
      // LOF基金搜索
      const searchResults = await apiService.searchFundsLOF(keyword);
      // 限制只显示前10个结果
      results.value = searchResults.slice(0, 20);
    }
  } catch (error) {
